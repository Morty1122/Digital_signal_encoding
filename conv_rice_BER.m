function [BER] = conv_rice_BER(msg, CtLength, CdGener)
%% 卷积码编码
...
for j = 1:len_EbN0                                  % 根据不同信噪比加入噪声
    q = 10;
    h = sqrt(q/(q+1)) + sqrt(1/(q+1)).*(1/sqrt(2) .* (randn(1,n*length(msg)) + 1i.*randn(1,n*length(msg))));
...
%% 作图
BER_theor_Rice = 0.5*(1-sqrt(EbN0./(1+EbN0)));
% semilogy(EbN0dB, BER_theor_Rice, EbN0dB,BER, 'bp-','Linewidth', 2);
